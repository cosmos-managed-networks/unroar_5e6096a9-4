...
HTTPS_GIT := github.com/shivhg/unroar.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=unroar \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=unroar \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/unroar ./cmd/unroar
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/unroar.exe ./cmd/unroar
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/unroar
...
draw-deps:
	...
	@goviz -i ./cmd/unroar -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c unroar --local -f chains.yaml
endif
...
set-testnet-configs:
	unroar config set client chain-id chainid-1
	unroar config set client keyring-backend test
	unroar config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | unroar keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | unroar keys add acc1 --recover`
