...
# then log output of file /code/bin/unroar
# then ensure static linking
RUN LEDGER_ENABLED=false BUILD_TAGS=muslc LINK_STATICALLY=true make build \
  && file /code/build/unroar \
  && echo "Ensuring binary is statically linked ..." \
  && (file /code/build/unroar | grep "statically linked")

# --------------------------------------------------------
FROM alpine:3.16

COPY --from=go-builder /code/build/unroar /usr/bin/unroar
